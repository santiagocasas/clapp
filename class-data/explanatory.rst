...
* 
    ...
    | 1) List of requested observables (besides those related to the background and thermodynamics,
    | which are always computed). The full list of possible keywords is:
    ...
    | --> convergence kappa and shear gamma: they share the same harmonic
    ...
    | code considerably.
    |
    | Based on your requests for the 'output' field, the code will
    | choose to either evaluate or skip each of its main modules. If
    | 'output' is left blank, only the background and thermodynamics
    | modules are evaluated, and no perturbations are computed.
    |
    | If you run the C code in a terminal and want some output files
    | for further quantities related to:
    | - the background evolution,
    | - the thermal history,
    | - the evolution of perturbations as a function of time for a few fixed wavenumbers,
    | - the primordial spectrum,
    | - an exotic energy injection history,
    | - the detailed CMB spectral distorsions,
    | do not set things here with the 'output' field. For such output files, there
    | is a series of dedicated flags, set with a syntax 'write_... = yes',
    | which are listed further down in the section on 'Output parameters',
    | items 1.d) to 1.k).
    |
    | (default: 'output' set to blank, no perturbation calculation)
...
* 
    ...
    | 1) Photon density: either 'T_cmb' in K, or 'Omega_g', or 'omega_g' (with omega_g = Omega_g h**2)
    | (default: 'T_cmb' set to 2.7255)
...
* 
    ...
    | 2) Baryon density: either 'Omega_b', or 'omega_b' (with omega_b = Omega_b h**2)
    | (default: 'omega_b' set to 0.02238280)
...
* 
    ...
    | 'Neff' (no underscore) to fix the total effective number of
    | neutrinos in the early universe, or one of 'N_ur', 'Omega_ur', 'omega_ur'
    | (with omega_ur = Omega_ur h**2) to fix only the density of massless species
    | (default: 'N_ur' set to 3.044. This value is more accurate than the previous
    | default value of 3.046, see 2008.01074 and 2012.02726)
    |
    | 3a) If you choose to pass 'Neff', a shooting algorithm will find the
    | correct value of N_ur (taking into the contribution of non-cold dark
    | matter species to Neff).
    |
...
* 
    ...
    | 4) Density of cdm (cold dark matter): you may specify:
    | - the density of cdm only, 'Omega_cdm' or 'omega_cdm'
    | (with omega_cdm = Omega_cdm h**2),
    | - or the density of total non-relativistic matter: 'Omega_m' or 'omega_m'
    | (with omega_m = Omega_m h**2).
    | If you pass 'Omega_m' or 'omega_m', the code will automatically fill
    | up the density of cdm such that all non-relativistic species
...
* 
    ...
    | 5.b) 'use_ncdm_psd_files' is a list of N_ncdm booleans (0 or 1):
    | - 0 means 'phase-space distribution (psd) passed analytically
    | inside the code, in the module background.c, inside the function
    | background_ncdm_distribution()', and by default, Fermi-Dirac,
...
* 
    ...
    | 5.d) 'Omega_ncdm' or 'omega_ncdm' (with omega_ncdm = Omega_ncdm h**2) or
    | 'm_ncdm' in eV (default: all set to zero);
    | with only one of these inputs, CLASS computes the correct value
...
* 
    ...
    | For standard active neutrinos you would usually choose between two schemes:
    |
    | - If you want to us massive neutrinos including the small
    | non-thermal corrections predicted by the standard neutrino
    | decoupling model, which gives a factor M_nu/omega_nu of 93.14 eV,
    | set 'T_ncdm' to 0.71611 (recommended for optimal accuracy).
    | Then each massive neutrino will contribute to Neff as 1.013.
    |
    | - Instead, if you use massive neutrinos normalised to give a
    | contribution to Neff of 1 each, and a factor M_nu/omega_nu of 94.1 eV
    | (instantaneous decoupling limit, less realistic and accurate),
    | set 'T_ncdm' to (4/11)^(1/3)=0.71377.
    |
    | If you switch between these two schemes while enforcing
    | consistently the same value of Neff (and thus different values
    | of N_ur), you will get small differences, but these would be
    | hard to discriminate with the next generation of cosmological
    | observations.
    |
    | (default: first scheme, 'T_ncdm' set to 0.71611)
...
* 
    ...
    | 7.1) Decaying CDM into Dark Radiation: you may pass either:
    |
...
* 
    ...
    | and its relativistic decay radiation):
    | 'Omega_dcdmdr' or 'omega_dcdmdr' (with omega_dcdmdr = Omega_dcdmdr h**2)
...
* 
    ...
    | 7.1.b) or, the rescaled initial value for dcdm density (see 1407.2418 for definitions):
    | 'Omega_ini_dcdm' or 'omega_ini_dcdm' (with omega_ini_dcdm = Omega_ini_dcdm h**2)
    | If you specify 7.a.1, 7.a.2 will be found automtically by a shooting method, and vice versa.
    | (default: 'Omega_dcdmdr' set to 0, hence so is 'Omega_ini_dcdm')
...
* 
    ...
    | Can be passed as either 'f_idm' (fraction), 'Omega_idm', or 'omega_idm'
    | (with omega_idm = Omega_idm h**2)
    | (default : 0)
